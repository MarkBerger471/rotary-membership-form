...
function syncRecipients(settings, { email, previousEmail, name, shouldReceive, waNumber }) {
  ...
  if (shouldReceive && target) {
    ...
    if (existing) {
      ...
      // The directory has their number, so the Recipients page does not have
      // to be told it again - ticking WhatsApp is then one click. A number
      // typed on that page by hand is never overwritten from here.
      if (waNumber && !String(existing.waNumber || '').trim()) {
        existing.waNumber = waNumber;
        changed = true;
      }
    } else {
      // Email only to begin with, which is how the board has always been
      // asked. WhatsApp and LINE are ticked per person on the Recipients page,
      // deliberately - nobody starts being messaged because they joined a board.
      next.push({
        email: email.trim(),
...
async function saveWithRecipientSync(members, member, previousEmail) {
  ...
  const { recipients, changed } = syncRecipients(settings, {
    ...
    shouldReceive,
    waNumber: toE164(member.whatsapp || member.phone),
  });
