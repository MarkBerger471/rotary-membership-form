...
// Who was asked to vote on this application. The board used to be asked only
// by email, so that is what the list was called; it can now be asked on
// WhatsApp and LINE too, and askedTo is the union. An application opened
// before that - one whose poll is running right now - has only emailedTo, and
// must go on being counted against exactly the people it was sent to.
function askedList(app) {
  if (Array.isArray(app && app.askedTo)) return app.askedTo;
  return Array.isArray(app && app.emailedTo) ? app.emailedTo : [];
}

// How each of them was asked: { 'someone@example.com': ['email','whatsapp'] }.
// Absent for older applications, where the answer is email, and only email.
function askedVia(app, email) {
  const map = app && app.askedVia;
  const norm = (e) => (e || '').trim().toLowerCase();
  if (map && typeof map === 'object') {
    const hit = Object.keys(map).find(k => norm(k) === norm(email));
    if (hit && Array.isArray(map[hit])) return map[hit];
  }
  return askedList(app).some(e => norm(e) === norm(email)) ? ['email'] : [];
}

// Split the people who were asked to vote into how they answered.
function tally(app, votes) {
  const asked = askedList(app);
  const approved = [];
  const rejected = [];
  const pending = [];
