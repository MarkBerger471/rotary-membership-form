...
// A message has no button to hide a URL behind, so the URL is what the board
// reads. The long form - id, address, name, action, channel - ran to three
// wrapped lines each and made the whole message unreadable, so a message
// carries a short link that stands for all of it.
//
// The token is derived rather than random: the same person, the same
// application and the same channel always produce the same one, so re-queueing
// or reminding does not scatter a new link each time. It is a capability, the
// same as the long link was - whoever holds it votes as that person.
const TOKEN_KEY = (token) => `vote:t:${token}`;

function tokenFor(appId, email, via) {
  const secret = process.env.ADMIN_PASSWORD || process.env.CRON_SECRET || 'rcbd-vote';
  return crypto
    .createHmac('sha256', secret)
    .update(`${appId}|${String(email || '').trim().toLowerCase()}|${via || ''}`)
    .digest('base64url')
    .slice(0, 12);
}

// Writes the token down so the vote function can read it back. Idempotent.
async function ensureToken(appId, email, via) {
  const token = tokenFor(appId, email, via);
  await kv.set(TOKEN_KEY(token), { appId, email, via });
  return token;
}

async function resolveToken(token) {
  try {
    const found = await kv.get(TOKEN_KEY(String(token || '')));
    return found && found.appId && found.email ? found : null;
  } catch (err) {
    console.error('Could not read vote token:', err);
    return null;
  }
}
...
//
// Laid out the way the email lays out its buttons - a label, then the thing
// itself - because a bare URL in the middle of a sentence is the part that
// made these messages look like spam. Plain text throughout: WhatsApp would
// take *bold*, LINE has no formatting at all and would show the asterisks.
function linkBlock(app, email, name, channel, hasFiles, token) {
  const url = token
    ? (what) => links.shortUrl(token, what)
    : (what) => (what === 'a' || what === 'r'
        ? links.voteUrl(app.id, email, name, what === 'a' ? 'approve' : 'reject', channel)
        : links.fileUrl(app.id, email, what));

  const block = [];
  if (hasFiles) block.push(`\u{1F4C4} The application\n${url('pdf')}`);
  block.push(`\u2705 Approve\n${url('a')}`);
  block.push(`\u274C Reject\n${url('r')}`);
  return block.join('\n\n');
}
...
// Trailing spaces left by a placeholder that came out empty, and the gaps a
...
// The wording Mark edits on the admin page. A message whose links have been
// edited out is one nobody can answer, so they go back on the end rather than
// going out without them.
function render(template, vars) {
  let out = String(template == null || template === '' ? settingsLib.DEFAULT_MESSAGE_TEMPLATE : template);
  Object.keys(vars).forEach(key => { out = out.split(`{{${key}}}`).join(vars[key]); });
  if (vars.links && !out.includes(vars.links)) out = out.trimEnd() + '\n\n' + vars.links;
  return tidy(out);
}
...
// takes one tap.
function textFor({ kind, app, recipient, channel, closeDays, daysLeft, result, tally, template, token }) {
  ...
  const linkLines = linkBlock(app, recipient.email, recipient.name, channel, hasFiles, token);
  ...
  if (kind.startsWith('reminder')) {
    ...
    return tidy([
      `Rotary Club Bangkok DACH`,
      `Still waiting on your vote`,
      ``,
      `${greeting}the application from ${who} has not had your answer yet${left ? `, and it closes ${left}` : ''}.`,
      ``,
      linkLines,
      ``,
  ...
  return render(template, {
    ...
    name: who,
    links: linkLines,
    closes: closing ? `Voting closes on ${closing}.` : '',
  });
}

// ------------------------------------------------------------------- asking

// Write the WhatsApp and LINE messages for one application. `recipients` is the
// board as lib/board resolved it - every address already read from the member
// directory. `only` narrows it to particular addresses: a resend, or a reminder
// to whoever has not voted. Returns the addresses that now have a message
// waiting, and on what.
//
...
// not be queued.
async function askBoard(recipients, app, options = {}) {
  const { kind = 'ask', only = null, closeDays, daysLeft, result, tally, template } = options;
  ...
  CHANNELS.forEach(channel => {
    board.on(recipients, channel).forEach(r => {
      if (wanted && !wanted.includes(norm(r.email))) return;
      targets.push({ channel, recipient: r });
  ...
  for (const { channel, recipient } of targets) {
    // The short link this message will carry. If it cannot be written down the
    // message still goes, with the long links - unreadable beats unsent.
    let token = null;
    try {
      token = await ensureToken(app.id, recipient.email, channel);
    } catch (err) {
      console.error(`Could not write a vote link for ${recipient.email}:`, err);
    }
    entries.push({
      id: 'ob_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2, 7),
      key: keyFor(app.id, recipient.email, channel, kind),
      channel,
      kind,
      appId: app.id,
      appName: app.name || '',
      ...
      waNumber: String(recipient.waNumber || '').trim(),
      lineName: String(recipient.lineName || '').trim(),
      text: textFor({ kind, app, recipient, channel, closeDays, daysLeft, result, tally, template, token }),
...
// A message to somebody who is neither a guest nor a board member - the person
// who takes the numbers for an evening, say. It rides the same queue and the
// same senders; only the wording is decided elsewhere.
//
// Sent twice for the same meeting, the newer list replaces the older one while
// it is still waiting: what the restaurant wants is the current list, not two
// of them.
async function queueNote({ channel, key, name, lineName, waNumber, text, about, group }) {
  if (!CHANNELS.includes(channel)) throw new Error(`Unknown channel "${channel}"`);
  if (!String(text || '').trim()) throw new Error('A note needs something to say');
  ...
  const entry = {
    ...
    channel,
    kind: 'note',
    // Whether a LINE group is an acceptable target. A note to whoever takes
    // the numbers usually goes to one; a board vote never may.
    group: !!group,
    appName: String(about || ''),
    name: String(name || lineName || waNumber || '').trim(),
  ...
  const waiting = entry.key && list.find(e => isPending(e) && e.key === entry.key);
  if (waiting) {
    Object.assign(waiting, { text: entry.text, name: entry.name, lineName: entry.lineName, waNumber: entry.waNumber, group: entry.group, queuedAt: entry.queuedAt });
